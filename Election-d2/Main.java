public class Main {
    ...
    public static void main(String[] args) {
        ...
        int[][] electionDataIntArray= parseData(electionDataRaw);
         ...
         treas1= David Gassner
         treas2= Sharon Bennett
         treas3= Walt Ritscher
         treas4= Heather Hurley
         */

        //create 2D string array of candidates, starting off with the office each group is running for
        String[][] candidatesStrArray = {{"President", "Mordy Golding", "Shira Gotshalk", "Stephanie Evans", "Ray Villalobos"}, {"Vice President", "Dona Sarkar", "Tyler Ahn", "Joe Camp", "Brandon LeBlanc"}, {"Secretary", "Jason Howard", "Nikki Rubino", "Peggy Fisher", "Neil Edde"}, {"Treasurer", "David Gassner", "Sharon Bennett", "Walt Ritscher", "Heather Hurley"}};

        //Defer to repeatIDCheck method
        electionDataIntArray = repeatIDCheck(electionDataIntArray);
        for (int i= 0; i < candidatesStrArray.length; i++) {
            System.out.printf("%s\n", voteCounter(electionDataIntArray, candidatesStrArray, i));
        }
    }

    public static int[][] parseData(String parseData) {
        //Create 2D string array with [rows][columns]
        String[][] electionDataStrArray = new String[5][250];

        //Create string array to split the data by semi-colon
        String[] splitByVoter = parseData.split(";");
        for (int i= 0; i < splitByVoter.length; i++) {
            //Create new 2D string arrays from parsed data based on index number
            electionDataStrArray[0][i] = splitByVoter[i].substring(0, 6);
            electionDataStrArray[1][i] = splitByVoter[i].substring(7, 8);
            electionDataStrArray[2][i] = splitByVoter[i].substring(9, 10);
            electionDataStrArray[3][i] = splitByVoter[i].substring(11, 12);
            electionDataStrArray[4][i] = splitByVoter[i].substring(13);
        }

        int[][] electionDataIntArray = new int[5][250];
        for (int j= 0; j < electionDataIntArray.length; j++) {
            for (int k= 0; k < electionDataIntArray[j].length; k++) {
                electionDataIntArray[j][k] = Integer.parseInt(electionDataStrArray[j][k]);
            }
        }
        return electionDataIntArray;
    }

    //create method to determine if there are any duplicate voter IDs (meaning someone voted more than once). Those votes will be discarded
    public static int[][] repeatIDCheck(int[][]electionDataIntArray) {
        for (int i= 0; i < electionDataIntArray[0].length; i++) {
            for (int j= 0; j < electionDataIntArray[0].length; j++) {
                if (electionDataIntArray[0][i]== electionDataIntArray[0][j] && i!= j) {
                    for (int k= 1; k < electionDataIntArray.length; k++) {
                        electionDataIntArray[k][i]= 0;
                    }
                }
            }
        }
    ...
    //Create method to count the number of valid votes for each office (based on index number)
    public static String voteCounter(int[][] electionDataIntArray, String[][] candidatesStrArray, int office) {
        int[] votesNum= {0, 0, 0, 0, 0};
        for (int i= 0; i < votesNum.length; i++) {
            for (int j= 0; j < electionDataIntArray[office].length; j++) {
                if (electionDataIntArray[office + 1][j]== i) votesNum[i]++;
            }
        }
        String validVotesCounted = "Office of " + candidatesStrArray[office][0] + " ("+ Integer.toString(250 - votesNum[0])+ " votes):\n";
        for (int k = 1; k < candidatesStrArray[office].length; k++) {
            validVotesCounted += candidatesStrArray[office][k] + " received " + Integer.toString(votesNum[k]) + " votes.\n";
        }

        int indexNum= 0;
        int max= votesNum[0];
        for (int l = 0; l < votesNum.length; l++) {
            if (max < votesNum[l]) {
                max= votesNum[l];
                indexNum= l;
            }
        }
        validVotesCounted += candidatesStrArray[office][indexNum] + " won.\n";
        return validVotesCounted;
    }
}